...
public class MongoCommandRepository<T>(IMongoContext context) : ICommandRepository<T> where T : class
{
    ...
    public void Delete(IEnumerable<T> items) => items.ToList().ForEach(item => _collection.DeleteOne(Filters.Id<T>(GetKey(item))));
    ...
    public Task DeleteAsync(IEnumerable<T> items) => Task.WhenAll(items.Select(item => _collection.DeleteOneAsync(Filters.Id<T>(GetKey(item)))));
    ...
    private static object GetKey(object item) => item.GetType().GetProperty("Id")?.GetValue(item, null);
}
