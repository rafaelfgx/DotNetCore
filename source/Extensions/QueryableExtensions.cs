...
public static class QueryableExtensions
{
    ...
    {
        public IQueryable<T> Filter(string property, object value) => queryable.Filter(property, string.Empty, value);

        public IQueryable<T> Filter(string property, string comparison, object value)
        {
        ...
        public IQueryable<T> Order(string property, bool ascending)
        {
        ...
        public IQueryable<T> Page(int index, int size) => queryable is null || index <= 0 || size <= 0 ? queryable : queryable.Skip((index - 1) * size).Take(size);
    }
